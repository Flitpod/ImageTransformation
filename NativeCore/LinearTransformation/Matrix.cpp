...
namespace NativeCore
{
    ...
    Matrix::Matrix(int rows, int cols, const double* values) : m_Rows(rows), m_Cols(cols)
    {
        m_Values = std::vector<double>(rows * cols);
        for (int i = 0; i < m_Values.size(); i++)
        {
    ...
    Matrix::Matrix(int rows, int cols, const std::vector<double>& values) : m_Rows(rows), m_Cols(cols), m_Values(values)
    {
    }
    ...
    Matrix Matrix::GetAdjointMatrix() const
    {
        ...
        for (int i = 0; i < m_Rows; i++)
        {
            for (int j = 0; j < m_Cols; j++)
            {
                result(i, j) = std::pow(-1, i + j) * GetSubMatrixWithoutRowAndCol(i, j).GetDeterminant();
            }
        }
